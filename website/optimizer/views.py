...
def create_optimizer(request):
    if request.method == 'POST':
        ...
        if form2.is_valid() and 'change_slate' in request.POST:
            ...
            df = get_daily_roster('\\\\home\\ubuntu\\Fantasy-Fire\\website\\optimizer\\prediction.csv')
            # df = create_predictions(df)
            df = create_predictions(df,
                                    slate='\\\\home\\ubuntu\\Fantasy-Fire\\website\\optimizer\\Slates\\' + slate + '.csv')
            df['min_exposure'] = 0
            df['max_exposure'] = 100
        ...
        if form.is_valid():
            ...
            df = pd.read_csv("\\\\home\\ubuntu\\Fantasy-Fire\\website\\optimizer\\Slates\\" + slate + ".csv")
            df2 = pd.read_csv(r"\\home\ubuntu\Fantasy-Fire\website\optimizer\prediction.csv")
            ...
            exporter.export('\\\\home\\ubuntu\\Fantasy-Fire\\website\\optimizer\\lineups.csv')
