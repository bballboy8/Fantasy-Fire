...
def get_slates():
    slate_files = os.listdir(r'Fantasy-Fire\website\optimizer\Slates')
    slates = []
...
def create_optimizer(request):
    if request.method == 'POST':
        ...
        if form2.is_valid() and 'change_slate' in request.POST:
            ...
            df = get_daily_roster('Fantasy-Fire\\website\\optimizer\\prediction.csv')
            # df = create_predictions(df)
            df = create_predictions(df,
                                    slate='Fantasy-Fire\\website\\optimizer\\Slates\\' + slate + '.csv')
            df['min_exposure'] = 0
            df['max_exposure'] = 100
        ...
        if form.is_valid():
            ...
            df = pd.read_csv("Fantasy-Fire\\website\\optimizer\\Slates\\" + slate + ".csv")
            df2 = pd.read_csv(r"Fantasy-Fire\website\optimizer\prediction.csv")
            ...
            result.to_csv("Fantasy-Fire\\website\\optimizer\\Predictions.csv")
            optimizer.load_players_from_csv(
                'Fantasy-Fire\\website\\optimizer\\Predictions.csv')
            ...
            exporter.export('Fantasy-Fire\\website\\optimizer\\lineups.csv')

            with open('Fantasy-Fire\\website\\optimizer\\lineups.csv') as myfile:
        ...
        # df = create_predictions(df)
        df = create_predictions(df,
                                slate='Fantasy-Fire\\website\\optimizer\\Slates\\Main_Slate.csv')
        df['min_exposure'] = 0
        df['max_exposure'] = 100
        df.insert(0, "Include", '', True)
        for ind in df.index:
