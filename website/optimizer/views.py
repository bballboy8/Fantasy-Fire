...
def create_optimizer(request):
    if request.method == 'POST':
        ...
        if form2.is_valid() and 'change_slate' in request.POST:
            ...
            for ind in df.index:
                df['Predicted_FP'][ind] = "<input type='number' form='optimizer' step='any' name='fantasy_points_" + str(
                ...
                df['Min Exposure'][ind] = "<input type='number' step='any' name='min_exposure_" + str(ind) + "' value=" + str(
                    0) + ">"
                df['Max Exposure'][ind] = "<input type='number' step='any' name='max_exposure_" + str(ind) + "' value=" + str(
                    1) + ">"
        ...
        df.insert(0, "Include", '', True)
        for ind in df.index:
            df['Predicted_FP'][ind] = "<input type='number' step='any' form='optimizer' name='fantasy_points_" + str(
                ind) + "' value=" + str(round(df['Predicted_FP'][ind], 2)) + " id='id_predicted_fp'>"
            # df['Predicted FP'] = df['Predicted FP'].apply(lambda x: round(float(x), 2))
            # df['Predicted FP'] = df['Predicted FP'].apply(
            #     lambda x: "<input type='text' form='optimizer' name='fantasy_points_" + df['Name'] + "' value=" + str(
